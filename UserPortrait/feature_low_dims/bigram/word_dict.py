...
#     stopword_set = tools.get_stop_words("../../offline/stopwords.csv")
#     # feature_list = np.zeros((tools.get_lines(dataset), (length + 1) * unit_size))
#     feature_list = np.zeros((tools.get_lines(dataset), length * unit_size))
#
#     cnt = 0
#     for line in infile:
#         line_parts = line.strip().split("\t")
...
#
#         # feature = np.zeros((length + 1, unit_size))
#         feature = np.zeros((length, unit_size))
#         for word_pos in text:
#             if word_pos not in word_index_map:
#                 continue
#
...
def get_tf_or_rate_feature_bigram(dataset, text_index, tf_or_rate_map, pos_list):
    infile = open(dataset)
    length = len(pos_list)
    x_length = xrange(length)
    ...
    for word in words_set:
        ...
        for label, wordrate_map in wordrate_label_map.items():
            if word in wordrate_map:
                ...
                if pos == "":
                    continue
                if pos in pos_wordcnt_map:
                    pos_wordcnt_map[pos] += 1
                else:
                    pos_wordcnt_map[pos] = 1

    outfile = open("dat/word_rate_bigram.csv", "w")
    pos_wordcnt_tuple = sorted(pos_wordcnt_map.items(), key=lambda x: x[1], reverse=True)
    for pos, wordcnt in pos_wordcnt_tuple:
        outfile.write(pos + "\t" + str(wordcnt) + "\n")
...
def get_words_set_bigram(dataset, text_index, bigram_pos_set, min_tf):
    tf_map = tools.cal_corpus_tf(dataset, text_index)
    words_set = set()
    for (word, tf) in tf_map.items():
        if tf > min_tf:
            pos = word.split(",")[1]
            if "-" in pos and pos not in bigram_pos_set:
                continue
            words_set.add(word)
