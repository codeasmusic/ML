...
if __name__ == "__main__":
    ...
    target_train_list = [
        ...
        # dir_path + "processed_train/biclass_separate/age_biclass" + train_flag + ".csv",

        dir_path + "processed_train/train_gender_apns_bigram.csv",

        dir_path + "processed_train/biclass_separate/edu_biclass.csv",
        # dir_path + "processed_train/biclass_separate/edu_biclass" + train_flag + ".csv",
    ]

    target_test = dir_path + "processed_test/TEST_segfile_apns_bigram.csv"
    # target_test = dir_path + "predictions/biclass_separate/biclass_" + lab_name + test_flag + ".csv"
    ...
    for i in xrange(len(label_name)):
        ...
        outfile = open("keywords.csv", "w")
        corpus_tf = tools.cal_corpus_tf(target_train, text_index=4)
        for word in keyword_list:
            outfile.write(word + "\t" + str(corpus_tf[word]) + "\n")
        outfile.close()
        ...
        vote_labels = stats.mode(predict_labels.astype(np.int))[0][0]

        predict_file = dir_path + "predictions/biclass_separate/biclass_" + lab_name + ".csv"
